...
func (s *server) routes() http.Handler {
	...
	// Arena（团战）接口
	mux.HandleFunc("POST /api/arenas", s.requireAdmin(s.handleCreateArena))
	mux.HandleFunc("GET /api/arenas", s.requireAdmin(s.handleListArenas))
	mux.HandleFunc("GET /api/arenas/{id}", s.requireAdmin(s.handleGetArena))
	mux.HandleFunc("POST /api/arenas/{id}/start", s.requireAdmin(s.handleStartArena))
	mux.HandleFunc("POST /api/arenas/{id}/vote", s.requireAdmin(s.handleCastVote))
	mux.HandleFunc("POST /api/arenas/{id}/cancel", s.requireAdmin(s.handleCancelArena))
	// 根路径 302 到团战主入口；后台降级为 /admin.html
	mux.HandleFunc("GET /{$}", func(w http.ResponseWriter, r *http.Request) {
		http.Redirect(w, r, "/arena.html", http.StatusFound)
	})
	mux.Handle("GET /", http.FileServerFS(static))
	// 应急：浏览器打开 ?token=XXX 直接 set session cookie 302 回首页。
	// 仅在 AdminToken 配置时启用；为本地 demo 准备。
	mux.HandleFunc("GET /auto-login", s.handleAutoLogin)
