...
-- Arena（团战 / 群战）：一个主题 + N 个 Agent 围战。
-- 没有"正方/反方"——所有 participant 平等。
-- 发言顺序：order_mode 决定是 random 还是 manual；manual_order 存具体序列
-- （即便是 random 也会在 start 时洗牌并落库，方便 replay）。
...
-- Arena 每轮发言：每条 round 是某个 participant 在某轮次的发言。
-- reply_to_id 指向被 @ 引用的上一条 round（可空）。
-- 复用 task_assignments 跑单轮派发：assignment_id 关联那条指派。
...
CREATE INDEX IF NOT EXISTS idx_arena_round_arena ON arena_rounds(arena_id, seq);
-- Arena 投票：每个 participant 1 票（不可投自己），投给最被说服的 agent。
-- PRIMARY KEY 防止重复投票。
CREATE TABLE IF NOT EXISTS arena_votes (
    ...
    PRIMARY KEY (arena_id, voter_agent)
);
`
...
// agentByID 按 ID 查单个 Agent；不存在返回 errAgentNotFound。
...
// agentsByIDs 批量查 Agent（ID 顺序按输入，不存在的 ID 跳过）。
func (s *store) agentsByIDs(ids []string) (map[string]Agent, error) {
	if len(ids) == 0 {
		return map[string]Agent{}, nil
	}
	q := `SELECT id, name, hostname, os, arch, ip, meta, created_at, last_seen FROM agents WHERE id IN (` +
